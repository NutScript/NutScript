...
function GM:CanPlayerTakeItem(client, item)
    if (IsValid(item.entity)) then
        ...
        if (item.entity.nutSteamID == client:SteamID() and item.entity.nutCharID ~= char:getID()) then
...
function GM:PlayerLoadedChar(client, character, lastChar)
    ...
    local charclass = character:getData("character_class", nil)

    if (character) then
        for _, v in pairs(nut.class.list) do
            if (v.faction == client:Team()) and v.isDefault and charclass == nil then
                character:setClass(v.index)
                break
            else
                character:joinClass(charclass)
                break
            end
        end
    end
...
function GM:PlayerSay(client, message)
    ...
    if (chatType == "ic") and (nut.command.parse(client, message)) then return "" end
...
function GM:PlayerCanHearPlayersVoice(listener, speaker)
    ...
    if (listener:GetPos():DistToSqr(speaker:GetPos()) > nut.config.squaredVoiceDistance) then return false, false end
...
function GM:CharacterPreSave(character)
    ...
    if (nut.config.get("class_saving")) and (IsValid(client)) then
        character:setData("character_class", character:getClass())
    end
end
...
function GM:CreateDefaultInventory(character)
    ...
    if (nut.inventory.types[invType]) then
        return nut.inventory.instance(invType, {
            char = charID
        })
...
function GM:NutScriptTablesLoaded()
    ...
    nut.db.query("ALTER TABLE nut_players ADD COLUMN _firstJoin DATETIME"):catch(ignore)
    nut.db.query("ALTER TABLE nut_players ADD COLUMN _lastJoin DATETIME"):catch(ignore)
    -- Add missing _quantity column for nut_item table.
    nut.db.query("ALTER TABLE nut_items ADD COLUMN _quantity INTEGER"):catch(ignore)
end
